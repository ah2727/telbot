...
class VisitorSkill(BaseSkill):
    ...
    def _build_intro(self, product_name: str, visitor_name: str | None) -> str:
        # نام کاربر اگر داریم، استفاده می‌کنیم برای حس صمیمیت
        if visitor_name:
        ...
        # تکنیک‌ها:
        # - معرفی کوتاه و شفاف
        # - گرفتن اجازه برای توضیح (permission-based selling)
        # - اشاره به منفعت (کم کردن دردسر ارتباط با مراجعین)
        # - سؤال باز برای شناخت نیاز
        return (
            f"{name_part}سلام، من دستیار فروش {product_name} هستم. "
            "کار من این است که ببینیم این ربات واقعاً می‌تواند بخشی از فشار پاسخ‌گویی و نوبت‌گیری را از روی دوش شما بردارد یا نه. "
            "اگر موافق باشید، در چند جمله بگویم {product_name} چه کاری انجام می‌دهد، بعد با هم بررسی کنیم مناسب کار شما هست یا نه. "
            "اول بفرمایید در چه نوع کسب‌وکاری فعال هستید و بیشتر از چه کانالی با مراجعین در تماس هستید؛ تلفن، واتساپ یا شبکه‌های اجتماعی؟"
        )
